...
def combat():
    encounterchance = random.randint(1,4)
    ...
    while True:
        ...
        if combatinput == "1":
            ...
            combatinput = input("What would you like to do?")
        if player.health <= 0:
            print("You die in the heat of the fight, womp womp")
            break
    ...
    while True:
        ...
        if choice == "1" and current_location == town:
            ...
            if (random.random() > 0.5):
                player.health *= 2
                print(f"You healthed and are healthy(ier) ({player.health})")
                if (player.health > 10000):
                    player.health = 0
                    print("Stop overdosing it's not good for your appendix")
            else:
                if (player.money > 0):
                    ouch = random.randint(1,player.money)
                    player.health += ouch
                    player.money -= ouch
                    print(f"You accidentally ate ${ouch} after confusing it for grass")
                else:
                    player.health = 0
                    print("You are the what experts call a \"silly moment\". BROKE.")
            time.sleep(1)
            clear()
